...
import AddToIncrement from "../../components/common/AddToIncrement";

const FormStepFormular = ({ ticketData, setTicketData, campingAreas }) => {
  const { cardNumber, cardName, expiryDate, setCardNumber, setCardName, setExpiryDate } = PaymentCardInput();
  ...
  return (
    <form
      ...
      <fieldset className="_step_2_select_camping_">
        ...
        <div className="_camping_area_ grid grid-cols-2 gap-m py-m">
          {campingAreas.map((area, index) => {
            ...
            const buttonClasses = `
        ${isLast && isOdd ? "col-span-2 halfround-right border-r-2 min-w-[80%] m-auto" : ""}
        ${
          !isLast && index % 2 === 0
            ? "halfround-right border-l-0" // styles odd buttons
            : "halfround-left border-r-0" // styles even buttons
        } 
        ${ticketData.camping_area === area.area ? "bg-foreground text-background" : "bg-background text-foreground"}
          text-center hover:bg-lightblue hover:text-white text-normal font-bold tracking-wide py-2xs
      `;
            return (
              <button className={buttonClasses} key={area.area} onClick={() => setTicketData({ ...ticketData, camping_area: area.area })}>
                {area.area}
                <div className=" text-xs font-normal tracking-none">Available: {area.available}</div>
              </button>
            );
          })}
        </div>
          ...
          <div className="_2-3_person_tent_ grid gap-s pb-s">
            <section className="flex justify-between items-center">
              <div>
                ...
                <p className="text-lightblue font-light italic">200kr per tent</p>
              </div>
              ...
              <AddToIncrement
                ...
                decrement={() =>
                  setTicketData({
                    ...ticketData,
                    two_person_tents: Math.max(0, ticketData.two_person_tents - 1),
                  })
                }
              />
            </section>
            <section className="flex justify-between items-center">
              <div>
                ...
                <p className="text-lightblue font-light italic">300kr per tent</p>
              </div>
              ...
              <AddToIncrement
                ...
                decrement={() =>
                  setTicketData({
                    ...ticketData,
                    three_person_tents: Math.max(0, ticketData.three_person_tents - 1),
                  })
                }
              />
            </section>
      ...
      <fieldset className="_step_3_ px-m">
        <h2 className="font-germania-one text-step-2 text-center before:w-1/2">Personal information</h2>
      ...
      <fieldset className="_step_4_ px-m">
        <h2 className="font-germania-one text-step-2 text-center before:w-1/2">Payment details</h2>
        ...
        <section className="underline-border pt-2xs grid grid-cols-[1fr,1fr] [&>*]:text-nowrap place-items-center [&>*]:hover:cursor-pointer">
          ...
          <div className="flex items-center gap-2xs">
            <input type="radio" id="payment-mobilepay" name="payment" value="mobilepay" className="_payment_option_ custom-radio" />
          ...
          <div className=" flex items-center gap-2xs">
            <input type="radio" id="payment-card" name="payment" value="card" className="_payment_option_ custom-radio" defaultChecked />
        ...
        <div className="_payment_layout_ ">
          ...
          <div id="card-payment-details grid grid-cols-[1fr]">
            ...
            <input type="text" inputMode="numeric" id="card-number" name="card-number" value={cardNumber} maxLength={19} onChange={(e) => setCardNumber(e.target.value)} />

            <label htmlFor="card-name">Cardholder Name:</label>
            <input type="text" id="card-name" name="card-name" value={cardName} onChange={(e) => setCardName(e.target.value)} />

            <section className="grid grid-cols-2 ">
              <label htmlFor="card-expiry" className="row-start-1 ">
                Expiry Date:
              </label>
              <input type="text" inputMode="numeric" id="card-expiry" name="card-expiry" maxLength={5} className="_uniform_input_ border-2 border-black border-r-0 bg-transparent row-start-2 px-xs" onChange={(e) => setExpiryDate(e.target.value)} value={expiryDate} placeholder="MM/YY" />

              <label htmlFor="card-cvc">Security code:</label>
              <input type="text" inputMode="numeric" id="card-cvc" name="card-cvc" maxLength="3" placeholder="***" />
            </section>
          </div>
        </div>
      </fieldset>
